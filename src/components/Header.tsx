    ...
    >
      ...
      >
        <div className="flex w-full items-center justify-between">
          <a
            href="#"
            className="flex flex-col group gap-0.5"
            aria-label="Lebanese Dental Clinic Home"
            onClick={() => setIsMenuOpen(false)}
          >
            <span
              className="font-serif text-2xl md:text-3xl font-normal tracking-tight text-[var(--accent-warm)] transition-all duration-700"
              style={{ fontFamily: "var(--font-serif)" }}
            >
              Labanees Dental
            </span>
            <span 
              className="font-sans text-[8px] md:text-[9px] font-medium tracking-[0.28em] text-[var(--accent-warm)] uppercase"
              style={{ fontFamily: "var(--font-sans)" }}
            >
              Luxury Dental Clinic in Muscat
            </span>
          </a>
          <nav
          ...
          >
            {navLinks.map(({ label, href }) => (
              <Magnetic key={label}>
                <a
                  href={href}
                  className={`link-underline whitespace-nowrap font-sans text-[10px] font-medium uppercase tracking-[0.18em] transition-all duration-500 hover:text-white py-2 px-0.5 ${
                    label === "Home"
                      ? "text-white after:scale-x-100"
                      : "text-white/85 hover:tracking-[0.22em]"
                  }`}
                  style={{ fontFamily: "var(--font-sans)" }}
                >
                  {label}
            ...
            <Magnetic>
              <a
                href="#booking"
                className="group relative overflow-hidden rounded-sm border border-[var(--accent-warm)] bg-transparent px-7 py-3 text-[10px] font-semibold uppercase tracking-[0.18em] text-white transition-all duration-500 hover:bg-[var(--accent-warm)] hover:text-white"
                style={{ fontFamily: "var(--font-sans)" }}
              >
                Book Appointment
              </a>
            </Magnetic>
            <div className="flex items-center gap-5 ml-4 border-l border-[var(--accent-warm)]/40 pl-8">
              <a 
                href="https://www.instagram.com/lebanese_dental_clinic/" 
                target="_blank" 
                rel="noopener noreferrer"
                className="text-[var(--accent-warm)]/80 hover:text-white hover:scale-110 transition-all duration-500"
